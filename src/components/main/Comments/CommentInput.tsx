    ...
    return (
        <div className={`flex items-center w-full`}>
            ...
            <div className="flex-grow">
                <input
                    {...rest}
                    className={`${isSubmitting && isLoading && 'opacity-50'} dark:bg-indigo-1100 dark:!border-gray-800 dark:text-black`}
                    type="text"
                    readOnly={isLoading || isSubmitting}
                    ref={ref}
                />
                {isUpdateMode && <span className="text-xs text-black-500 ml-2">Press Esc to Cancel</span>}
            </div>
        </div>
    );
});
