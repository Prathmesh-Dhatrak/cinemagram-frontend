...
const ChatBox: React.FC<IProps> = ({ user, target }) => {
    ...
    return (
        <div className={`bg-white  relative w-full h-full laptop:w-20rem laptop:shadow-lg laptop:rounded-t-xl laptop:bottom-0 laptop:right-24 laptop:border laptop:border-gray-400 dark:border-gray-800`}>
            <div className="flex  laptop:rounded-t-xl p-3  justify-between pb-3 border-b border-gray-200 dark:border-gray-800 dark:bg-indigo-1100">
                <Link className="dark:text-black" to={`/user/${target.username}`}>
                ...
                <div className="hidden laptop:flex laptop:items-center">
                    <div
                        className="post-option-toggle p-2 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-200 dark:text-black dark:hover:bg-indigo-1000"
                        title="Minimize Chat"
                        onClick={onMinimize}
                    >
            ...
            {/* --- MESSAGES BODY ---- */}
            <div className="min-h-80%  p-3 laptop:min-h-20rem max-h-85% laptop:max-h-80 bg-white overflow-y-scroll  scrollbar relative">
                ...
                {(!isLoading && target.chats.length === 0 && error) && (
                    <div className="flex flex-col items-center min-h-18rem justify-center py-2">
                        {error.status_code === 404 ? (
                            <>
                                <CommentOutlined className="text-black-300 text-6xl" />
                                <span className="text-black-400 mb-2">No messages.</span>
                                <span className="text-black-400 text-sm">Send a message to {target.username}</span>
                            </>
                        ) : (
                            <span className="text-black-400 mb-4">
                    ...
                    <>
                                ...
                                >
                                    <div className="flex flex-col">
                                        ...
                                        <div className={`flex pb-2 ${msg.isOwnMessage ? 'justify-end mr-8' : 'justify-start ml-8'}`}>
                                            {/* ---DATE ---- */}
                                            <span className={`text-black-400 text-1xs ${msg.isOwnMessage ? 'order-2' : 'order-1'}`}>
                                                {displayTime(msg.createdAt, true)}
                                            </span>
                                            {/* ---- SEEN ---- */}
                                            {(msg.isOwnMessage && msg.seen && i === target.chats.length - 1) && (
                                                <span className={`text-black-400 mx-2 text-1xs italic flex-grow ${msg.isOwnMessage ? 'order-1' : 'order-2'}`}>
                                                    &nbsp;Seen
                                                </span>
                                            )}
                                        </div>
                        ...
                        {isTyping && (
                            <div className="flex justify-center py-2">
                                <span className="text-xs text-black-400">{target.username} is typing...</span>
                            </div>
                        )}
                    </>
                )}
            </div>
            <div className="relative  p-3  bottom-0 left-0 bg-white dark:bg-indigo-1100 w-full flex border-t border-gray-200 dark:border-gray-800">
                <input
                    className="flex-grow !border-gray-400 !rounded-r-none !py-0 dark:bg-indigo-1000 dark:text-black dark:!border-gray-800"
                    type="text"
                ...
                />
                <button
                    className="!rounded-l-none flex items-center justify-center text-white"
