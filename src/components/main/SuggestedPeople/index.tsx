...
const SuggestedPeople: React.FC = () => {
    ...
    return (
        <div className="w-full bg-white dark:bg-indigo-1000 rounded-md shadow-lg overflow-hidden">
            <div className="px-4 py-4 flex justify-between mb-4 border-b-2 border-gray-600">
                <h4 className="dark:text-black">Suggested People</h4>
                <Link to={SUGGESTED_PEOPLE} className="text-xs underline dark:text-red-700">See all</Link>
            </div>
            {isLoading && (
            ...
            {(!isLoading && error) && (
                <div className="flex min-h-10rem items-center justify-center">
                    <span className="text-black-400 italic">
                    ...
                    <div className="relative flex items-center justify-between px-4 py-2">
                        <Link to={`/user/${user.username}`}>
                            <div className="flex items-center">
                                ...
                                <h6 className="mr-10 text-sm overflow-ellipsis overflow-hidden dark:text-black">{user.username}</h6>
                            </div>
                        </Link>
