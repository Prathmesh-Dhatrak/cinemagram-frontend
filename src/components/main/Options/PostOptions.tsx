...
const PostOptions: React.FC<IProps> = (props) => {
    ...
    return (
        <div className="relative z-10" id={`post_${props.post.id}`}>
            <div
                className="post-option-toggle p-2 rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-200 dark:text-black dark:hover:bg-indigo-1100"
            ...
            {isOpenOption && (
                <div className="w-60 flex flex-col bg-white dark:bg-indigo-1000 rounded-md shadow-lg overflow-hidden absolute top-8 right-3 border border-gray-200 dark:border-gray-800 divide-y divide-gray-100 dark:divide-gray-800">
                    {props.post.isOwnPost ? (
                        <>
                            <h4
                                className="p-4 flex items-center hover:bg-indigo-500 hover:text-white cursor-pointer dark:text-black"
                            ...
                            <h4
                                className="p-4 flex items-center bg-white hover:bg-indigo-500 hover:text-white cursor-pointer dark:text-black"
                        ...
                        <BookmarkButton postID={props.post.id} initBookmarkState={props.post.isBookmarked}>
                            {({ dispatchBookmark, isBookmarked, isLoading }) => (
                                <h4
                                    className="group p-4 flex items-center cursor-pointer dark:bg-white dark:text-black hover:bg-indigo-500 hover:text-white"
                                    onClick={dispatchBookmark}
                                >
                                    {isLoading
                                        ? <LoadingOutlined className="text-black-600 text-2xl p-2 dark:text-black group-hover:text-black" />
                                        : isBookmarked ? (
