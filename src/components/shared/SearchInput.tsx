...
const SearchInput: React.FC<IProps> = (props) => {
    ...
    return (
        <div className={`input-wrapper relative flex flex-col items-center ${props.inputClassName}`}>
            <SearchOutlined className="text-black-400 absolute left-3 top-3 z-50" />
            <input
                className="!border-gray-100 !pl-10 !py-2 dark:bg-white dark:!border-gray-800 dark:text-black"
                placeholder="Search..."
                type="text"
            ...
            {(searchInput && isVisibleSuggestion) && (
                <div className={` bg-white dark:bg-indigo-1000 border-2 border-black shadow-lg rounded-md w-full flex justify-center flex-col overflow-hidden ${props.floatingResult ? 'absolute top-12' : 'relative top-0'}`}>
                    {(!props.showNoResultMessage && !error) && (
                        <h6 className="p-4 text-xs border-b dark:text-black border-gray-100 dark:border-gray-800">Search Suggestion</h6>
                    )}
                    {(isSuggesting && !error) && (
                    ...
                    {(!isSuggesting && !error && suggestions.length !== 0) && suggestions.map((user) => (
                        <div
                            className="hover:bg-indigo-100 dark:bg-white dark:hover:bg-gray-300 p-2 cursor-pointer"
                            key={user.id}
                            onClick={() => onClickItem(user)}
                        >
                            <div className="flex items-center">
                                <Avatar url={user.profilePicture?.url} className="mr-2" />
                                <h6 className="mr-10 text-sm max-w-md overflow-ellipsis overflow-hidden dark:text-black">{user.username}</h6>
                            </div>
                        </div>
                    ))}
                    {(error && suggestions.length === 0 && props.showNoResultMessage) && (
                        <div className="flex items-center justify-center p-4">
                            <span className="text-black-400 text-sm italic">
