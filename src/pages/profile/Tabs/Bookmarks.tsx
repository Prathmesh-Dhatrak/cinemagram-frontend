        ...
        <div className="flex flex-col items-start justify-start w-full min-h-10rem">
            ...
            {(bookmarks.length !== 0 && !error) && (
                <div className="w-full space-y-4" ref={infiniteRef as React.RefObject<HTMLDivElement>}>
                    <h4 className="text-white mb-4 ml-4 mt-4 laptop:mt-0">Bookmarks</h4>
                        ...
                        {bookmarks.map(item => (
                            ...
                            >
                                <div key={item.id} className="h-24 flex justify-between bg-white dark:bg-indigo-1000 rounded-md shadow-lg overflow-hidden">
                                    <div className="flex justify-center items-center border-r-2 border-black rounded-md">
                                        <BookmarkButton postID={item.post.id} initBookmarkState={item.isBookmarked}>
                                            {({ dispatchBookmark, isBookmarked, isLoading }) => (
                                                ...
                                                >
                                                    {isLoading
                                                        ? <LoadingOutlined className="text-black-600 text-2xl p-2 dark:text-black" />
                                                        : isBookmarked ? (
                                    ...
                                    <Link
                                        className="flex flex-grow justify-between border border-transparent dark:hover:bg-indigo-1000 "
                                        key={item.id}
                                        to={`/post/${item.post.id}`}
                                    >
                                        <div className="flex-grow p-2 pb-4 max-w-sm">
                                            <h4 className="break-all overflow-hidden overflow-ellipsis h-8 laptop:h-12 dark:text-black">
                                                {item.post.description}
                                            </h4>
                                            <span className="text-xs text-gray-800 self-end">Bookmarked {dayjs(item.createdAt).fromNow()}</span>
                                        </div>
                                        <div
