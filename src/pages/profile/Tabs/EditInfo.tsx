...
const EditInfo: React.FC<IProps> = ({ isOwnProfile, profile }) => {
    ...
    return (!isOwnProfile && profile.username) ? <Redirect to={`/user/${profile.username}`} /> : (
        <div className="dark:bg-indigo-1000 rounded-md min-h-10rem shadow-lg">
            <h3 className=" p-4 text-black-500 rounded-t-md border-b-2 border-black dark:text-black bg-white">Edit Info</h3>
            <form className="p-4 pb-8 space-y-4 divide-y divide-gray-100 dark:divide-gray-800" onSubmit={handleSubmit}>
                {/* ---- FIRST NAME ------- */}
                <div className="flex flex-col py-2">
                    <label htmlFor="firstname" className="ml-4 text-red-600 font-bold mb-2">First Name</label>
                    <input
                        className="dark:bg-white dark:text-black dark:border-gray-800"
                        readOnly={isUpdating}
                        id="firstname"
                        type="text"
                        maxLength={50}
                ...
                {/* ---- LAST NAME ------- */}
                <div className="flex flex-col py-2">
                    <label htmlFor="lastname" className="ml-4 text-red-600 font-bold mb-2">Last Name</label>
                    <input
                        className="dark:bg-white dark:text-black dark:border-gray-800"
                        readOnly={isUpdating}
                        id="lastname"
                ...
                <div className="grid laptop:grid-cols-2">
                    <div className="flex flex-col py-2">
                        <label htmlFor="gender" className="ml-4 text-red-600 font-bold mb-2">Gender</label>
                        <select
                            className="dark:bg-white dark:text-black dark:border-gray-800"
                            id="gender"
                    ...
                    <div className="flex flex-col py-2">
                        <label htmlFor="birthday" className="ml-4 text-red-600 font-bold mb-2">Birthday (mm/dd/yyyy)</label>
                        <input
                            className="dark:bg-white dark:text-black dark:border-gray-800"
                            readOnly={isUpdating}
                            type="date"
                ...
                {/* ---- BIO ------- */}
                <div className="flex flex-col py-2">
                    <label htmlFor="bio" className="ml-4 text-red-600 font-bold mb-2">Bio</label>
                    <textarea
                        ...
                        value={field.bio}
                    />
                    <span className="text-xs text-red-600 font-bold block text-right">
                ...
                <div className="flex justify-between pt-8">
                    <button
                        ...
                        type="button"
                        onClick={handleBack}
                        className="button--muted !rounded-full dark:bg-white dark:text-black dark:hover:bg-gray-200"
                    >
