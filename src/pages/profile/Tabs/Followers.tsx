        ...
        <div className="w-full" ref={infiniteRef as React.RefObject<HTMLDivElement>}>
            ...
            {(!isLoading && followers.length === 0 && error) && (
                <div className="w-full min-h-10rem flex items-center justify-center">
                    <h6 className="text-white italic">{error?.error?.message || 'Something went wrong.'}</h6>
                </div>
            )}
            {followers.length !== 0 && (
                <div>
                    <h4 className="text-white mb-4 ml-4 mt-4 laptop:mt-0"> {followers.length} Followers</h4>
